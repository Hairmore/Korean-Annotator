...
class BiaffineDependencyModel(Model):
    ...
    def __init__(self,
                 ...
                 bert=None,
                 n_bert_layers=4,#原本是4
                 mix_dropout=.0,
                 bert_pooling='mean',
                 bert_pad_index=0,
                 finetune=False,  #原本是false
                 n_plm_embed=0,
                 embed_dropout=.33,
                 n_encoder_hidden=800,#原本800
                 n_encoder_layers=3, #原本是3
                 encoder_dropout=.33,
                 n_arc_mlp=500,#原本是500
                 n_rel_mlp=100,
                 mlp_dropout=.38,
                 scale=0,
                 pad_index=0,
    ...
    def loss(self, s_arc, s_rel, arcs, rels, mask, partial=False):
        ...
        max_index = s_rel.shape[1] - 1  # Max index for the second dimension of s_rel
        arcs = torch.clamp(arcs, min=0, max=max_index)
        #print(arcs, rels)
        #print(arcs.shape, rels.shape)
